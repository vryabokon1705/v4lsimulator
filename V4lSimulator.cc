...
class streamer::CameraVideoRecorderPrivate
{
  ...
  /// \brief Video encoder
  public: V4lVideoEncoder videoEncoder;
    ...
    msgs::Boolean &_res)
{
  ...
  if (this->recordVideo)
  {
    ...
    else
    {
      ...
      if (this->recordVideoFormat != "v4l2")
      {
        gzerr << "Video encoding format: '" << this->recordVideoFormat
               << "' not supported. Available format is v4l2."
               << std::endl;
        _res.set_data(false);
        return true;
      }
...
//////////////////////////////////////////////////
void V4lSimulator::PostUpdate(const UpdateInfo &_info,
...
GZ_ADD_PLUGIN(streamer::V4lSimulator,
                    gz::sim::System,
                    streamer::V4lSimulator::ISystemConfigure,
                    streamer::V4lSimulator::ISystemPostUpdate)

// Add plugin alias so that we can refer to the plugin without the version
// namespace
GZ_ADD_PLUGIN_ALIAS(streamer::V4lSimulator, "streamer::V4lSimulator")
